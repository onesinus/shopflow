...
async function getDashboardStats() {
  ...
  const [revenueTotal] = await sequelize.query(
    `SELECT COALESCE(SUM(total_cents), 0) AS total FROM orders WHERE payment_status IN ('paid')`
  );

  const [revenueToday] = await sequelize.query(
    `SELECT COALESCE(SUM(total_cents), 0) AS total FROM orders WHERE payment_status IN ('paid') AND date(created_at) = '${today}'`
  );
